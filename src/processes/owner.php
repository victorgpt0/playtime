...
class owner
{
    ...
    public function deleteFacility($conn)
    {
        if (isset($_POST['deleteFacility'])) {
            $Derrors = [];
            print "<div class='container' id='main-content'>";

            $fid = $conn->escape_values($_POST['itemId']);
            ...
            if (!count($Derrors)) {
                ...
                try {
                    ...
                    if ($conn->deleteRecord('tbl_facilities', ['facilityId' => $fid]) === true) {
                        error_log("\nSuccess removing from DB ", 3, 'errors/error.log');
                    } else {
                        error_log("\nFailure at owner.php delete($fid)", 3, 'errors/error.log');
                    }
                } catch (Exception $e) {
